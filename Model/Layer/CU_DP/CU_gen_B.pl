...
///////////advanced parameters//////////
    ...
    reg  [$clog2($ceil_NUMBER_OF_FILTERS_over_NUMBER_OF_UNITS)-1 : 0] psums_counter_next; //$ceil_NUMBER_OF_FILTERS_over_NUMBER_OF_UNITS is ceil_NUMBER_OF_FILTERS_over_NUMBER_OF_UNITS
    ...
    begin
        ...
        else if(psums_counter_next == ( ($ceil_NUMBER_OF_FILTERS_over_NUMBER_OF_UNITS) -1) & ifm_sel_next_tick)
            psums_counter_next <= 0 ;
        else if(ifm_sel_next_tick)
            psums_counter_next <= psums_counter_next + 1'b1;      
    end
    assign psums_counter_next_tick = (psums_counter_next == ( ($ceil_NUMBER_OF_FILTERS_over_NUMBER_OF_UNITS) -1) & ifm_sel_next_tick);
    
    assign accu_enable = ( psums_counter_next != 0 );
    assign relu_enable = ( psums_counter_next == ($ceil_NUMBER_OF_FILTERS_over_NUMBER_OF_UNITS) -1 );
...
print $fh <<"DONATE";   
    ...
    begin     
        ...
        begin
            start_to_next = 1'b0;
            mem_empty     = 1'b1;
            if( (psums_counter_next == ( ($ceil_NUMBER_OF_FILTERS_over_NUMBER_OF_UNITS) -1) & ifm_address_write_next_tick) )
                state_next2 = s1;          
        end
